...
def model_prediction(test_image):
    model = tf.keras.models.load_model("trained_model.h5") #to load the model
...
#PREDICT BUTTON
    if(st.button("Predict")):
       # st.balloons()
        st.snow()  #instead of baloon i can use snow also
...
#Reading Labels
        ...
        label = []
        for i in content:
            label.append(i[:-1]) #Append we are using to reduce the space by -1 in labels "apple " to "apple"
